...
export function SubjectWorkspace({ subjectId, level, studyState, onOpenTutor, onOpenGenerator, onOpenProgress }: SubjectWorkspaceProps) {
  ...
  return <div className="mx-auto max-w-[1040px] px-4 pb-12 pt-6 sm:px-6 lg:pt-9"><section style={subjectThemeStyle(subjectId)} className="subject-workspace-header rounded-3xl border bg-white/88 px-5 py-5 backdrop-blur-sm dark:!bg-slate-900 sm:px-6"><div className="flex items-start gap-4"><span className="subject-accent-icon flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border font-heading text-lg font-semibold">{subjectInitial(subjectId)}</span><div className="min-w-0 flex-1"><div className="subject-context-label mb-1 text-[12px] font-semibold uppercase tracking-[.08em]">{level === "OL" ? "Ordinary Level" : "Higher Level"}</div><h1 className="font-heading m-0 text-[30px] font-semibold leading-tight tracking-[-.02em] text-gray-900 dark:text-white sm:text-[34px]">{subject}</h1><p className="m-0 mt-1 text-[14px] leading-relaxed text-gray-500">Tutor, exam practice, and a clear view of what to improve.</p></div></div></section>
