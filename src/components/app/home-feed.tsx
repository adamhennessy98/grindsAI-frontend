...
function ContinuationCard({ subject, state, onContinue }: { subject: Subject; state: NonNullable<StudyStateBySubject[string]>; onContinue: () => void }) {
  ...
  return <section className="rounded-2xl border border-cyan-100 bg-[linear-gradient(135deg,rgba(236,254,255,.86),rgba(248,250,247,.92))] px-4 py-4 shadow-[0_14px_38px_-34px_rgba(8,145,178,.65)] dark:!border-cyan-900 dark:!bg-none dark:!bg-cyan-400/10 sm:px-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-center gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-cyan-500 text-[13px] font-semibold text-white">{subjectInitial(subject.id)}</span><div className="min-w-0"><div className="text-[12px] font-semibold uppercase tracking-[.08em] text-cyan-700 dark:text-cyan-300">Continue where you left off</div><div className="mt-1 truncate font-heading text-[17px] font-semibold text-gray-900 dark:text-white">{subjectLabel(subject.id)}: {detail}</div><p className="m-0 mt-1 text-[13px] text-gray-500">Pick up your subject-aware Tutor session.</p></div></div><button type="button" onClick={onContinue} className="shrink-0 rounded-xl bg-cyan-500 px-4 py-2.5 text-sm font-semibold text-white hover:bg-cyan-600">Continue</button></div></section>;
}
