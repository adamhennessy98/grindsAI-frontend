...
function TopicList({ topics, activeTopicId, onSelectTopic }: { topics: ReturnType<typeof getSubjectTopics>; activeTopicId?: string; onSelectTopic: (topicId: string) => void }) {
  return <><div className="mb-3 hidden px-2 lg:block"><div className="subject-context-label text-[10.5px] font-semibold">TUTOR TOPICS</div><div className="mt-1 text-[12.5px] text-gray-400">{topics.length} sections</div></div><div className="space-y-1">{topics.map((topic) => <button key={topic.id} type="button" aria-current={activeTopicId === topic.id ? "page" : undefined} onClick={() => onSelectTopic(topic.id)} className={`group relative flex w-full items-start gap-2.5 rounded-lg px-2.5 py-2.5 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 ${activeTopicId === topic.id ? "bg-cyan-50 text-cyan-800 dark:bg-cyan-400/12 dark:text-cyan-100" : "bg-transparent text-gray-700 hover:bg-gray-100 dark:text-slate-200 dark:hover:bg-slate-800/70"}`}><span className={`mt-[5px] h-1.5 w-1.5 shrink-0 rounded-full ${activeTopicId === topic.id ? "bg-cyan-500" : "bg-gray-300 group-hover:bg-cyan-400"}`} /><span className="min-w-0"><span className="block text-[13.5px] font-medium leading-snug">{topic.name}</span></span></button>)}</div></>;
}
