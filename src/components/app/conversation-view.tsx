    ...
    <div style={subjectThemeStyle(subjectId)} className="mx-auto grid h-[calc(100dvh-65px)] max-w-[1180px] grid-cols-1 overflow-hidden lg:grid-cols-[260px_minmax(0,1fr)]">
      ...
      <div className="flex min-h-0 min-w-0 flex-col px-4 sm:px-6">
        <div className="flex shrink-0 items-center gap-3 border-b border-gray-200 px-1 pb-3 pt-4 dark:border-slate-800">
          <button type="button" onClick={() => setTopicsOpen(true)} aria-label="Open tutor topics" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-gray-200 bg-white text-[12px] font-medium text-gray-600 transition-colors hover:border-cyan-500 hover:text-cyan-700 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-cyan-500/20 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 lg:hidden">Topics</button>
          ...
          <span className="hidden rounded-full border border-cyan-100 bg-cyan-50 px-3 py-1.5 text-xs text-cyan-700 dark:border-cyan-900 dark:bg-cyan-400/10 dark:text-cyan-200 sm:inline">Guided help</span>
        </div>
        ...
        <div className="shrink-0 border-t border-gray-100 bg-[#eaf1ed]/95 px-1 pb-[calc(1rem+env(safe-area-inset-bottom))] pt-3 backdrop-blur-sm dark:border-slate-800 dark:bg-slate-950/95">{(contextualActions.length > 0 || messages.length > 0 || handoff) && <div className="mb-2.5 flex flex-wrap gap-2">{contextualActions.map((action) => <QuickAction key={action} onClick={() => void sendMessage(action)}>{action}</QuickAction>)}{(messages.length > 0 || handoff) && <QuickAction onClick={onOpenGenerator}>Practise this topic</QuickAction>}</div>}<form onSubmit={(event) => { event.preventDefault(); void sendMessage(draft); }} className="flex items-center gap-2.5 rounded-[13px] border border-gray-200 bg-white py-1.5 pl-4 pr-1.5 shadow-[0_6px_18px_-14px_rgba(17,24,39,.5)] dark:border-slate-700 dark:bg-slate-900"><input value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Paste a question or tell your Tutor where you are stuck..." className="flex-1 border-none bg-transparent py-2.5 text-[14.5px] text-gray-700 outline-none placeholder:text-gray-400 dark:text-white dark:placeholder:text-slate-400" /><button type="submit" disabled={!draft.trim() || isResponding} aria-label="Send message" className="flex h-[38px] w-[38px] shrink-0 items-center justify-center rounded-[10px] bg-cyan-500 text-[12px] font-semibold text-white transition-colors hover:bg-cyan-600 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-cyan-500/20 disabled:bg-gray-300 dark:disabled:bg-slate-700">Send</button></form></div>
      </div>
    </div>
  );
}
