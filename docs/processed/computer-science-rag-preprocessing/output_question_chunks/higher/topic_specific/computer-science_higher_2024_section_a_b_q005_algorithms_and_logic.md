---
subject: "Computer Science"
subject_id: "computer-science"
level: "Higher"
year: 2024
paper: "Section A B"
question_number: 5
section: "Question 1"
topic: "Algorithms and Logic"
secondary_topics: []
classification_type: "topic_specific"
source_type: "past_paper_with_marking_scheme"
has_visual: true
visual_assets:
  - "../../../image_assets/exam_papers/higher/computer_science_higher_2024_section_a_b_exam_page_003.png"
source_exam_pages:
  - "3"
source_marking_scheme_pages:
  - "4"
pairing_confidence: "high"
topic_confidence: "medium"
classification_source: "fallback_rules"
candidate_topics:
  - "Algorithms and Logic"
  - "Computer Systems"
needs_review: false
review_reason: ""
---


# Question

Question 1

Logic gates have one or more inputs and a single output. For each logic gate in Column A in the
table below enter the output, either 0 or 1, in Column B.

               Column A                              Column B
            Logic gate with input(s)                        Output (0 or 1)

# Marking Scheme

Question 1                                                                6 marks

               Column A                              Column B
            Logic gate with input(s)                            Output


                                                           1



                                                           0



                                                           0



                                                           1



                                                           0



                                                           0


                             Each correct response 1 mark

# Source References

Exam paper:
- file: intermediate_markdown/exam_papers/higher/computer_science_higher_2024_section_a_b_exam.md
- pages: [3]

Marking scheme:
- file: intermediate_markdown/marking_schemes/higher/computer_science_higher_2024_paper_1_marking_scheme.md
- pages: [4]

# Notes

