---
subject: "Computer Science"
subject_id: "computer-science"
level: "Higher"
year: 2024
paper: "Section A B"
question_number: 9
section: "1. Move into an empty square one position to the left or right, for example 1 \uf0e0 2 means move"
topic: "Computer Systems"
secondary_topics: []
classification_type: "topic_specific"
source_type: "past_paper_with_marking_scheme"
has_visual: true
visual_assets:
  - "../../../image_assets/exam_papers/higher/computer_science_higher_2024_section_a_b_exam_page_004.png"
source_exam_pages:
  - "4"
source_marking_scheme_pages:
  - "4"
pairing_confidence: "high"
topic_confidence: "medium"
classification_source: "fallback_rules"
candidate_topics:
  - "Computer Systems"
  - "Algorithms and Logic"
needs_review: false
review_reason: ""
---


# Question

1. Move into an empty square one position to the left or right, for example 1  2 means move
   the disc from square 1 to square 2.

# Marking Scheme

Question 1                                                                6 marks

               Column A                              Column B
            Logic gate with input(s)                            Output


                                                           1



                                                           0



                                                           0



                                                           1



                                                           0



                                                           0


                             Each correct response 1 mark

# Source References

Exam paper:
- file: intermediate_markdown/exam_papers/higher/computer_science_higher_2024_section_a_b_exam.md
- pages: [4]

Marking scheme:
- file: intermediate_markdown/marking_schemes/higher/computer_science_higher_2024_paper_1_marking_scheme.md
- pages: [4]

# Notes

